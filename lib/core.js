...
function migrateConfig(config) {
    ...
    // 粉丝群组（独立页面 /groups.html）：配置 + 一个仅用于导航占位的模块
    if (!config.fanGroups || typeof config.fanGroups !== 'object') config.fanGroups = {};
    if (typeof config.fanGroups.heading !== 'string') config.fanGroups.heading = '粉丝群组';
    if (typeof config.fanGroups.desc !== 'string') config.fanGroups.desc = '';
    if (!Array.isArray(config.fanGroups.groups)) config.fanGroups.groups = [];
    ...
    // 粉丝群组模块项兜底：老配置补一个 nav 占位模块（插在 footer 之前，保证导航顺序在「行程」之后）
    if (Array.isArray(config.modules) && !config.modules.some(m => m && m.type === 'fanGroups')) {
        const fi = config.modules.findIndex(m => m && m.type === 'footer');
        const item = { type: 'fanGroups', visible: true };
        if (fi >= 0) config.modules.splice(fi, 0, item); else config.modules.push(item);
    }
...
const DEFAULT_CONFIG = {
    ...
    fanGroups: { heading: '粉丝群组', desc: '', groups: [] },
    modules: [
        { type: 'hero', visible: true },
        { type: 'about', visible: true },
        { type: 'works', visible: true },
