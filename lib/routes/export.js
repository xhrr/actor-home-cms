...
module.exports = function createExportRoutes(core) {
    ...
    router.post('/api/export', (req, res) => {
        try {
            ...
            const commentsJs = 'window.SITE_CONFIG = window.SITE_CONFIG || {};\nwindow.SITE_CONFIG.comments = ' + JSON.stringify(config.comments || {}, null, 4) + ';\n'
                // 评论功能一键开关（config.commentSettings.enabled，缺省开启）：前台读取后零渲染
                + 'window.SITE_CONFIG.commentsEnabled = ' + (!(config.commentSettings && config.commentSettings.enabled === false) ? 'true' : 'false') + ';\n';
