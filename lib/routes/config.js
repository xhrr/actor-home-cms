...
module.exports = function createConfigRoutes(core) {
    ...
    /**
     * 分片保存：只合并提交的顶层字段（sections，对象深合并）与模块数组（modules，整体替换）。
     * 各分区/插件数据互不覆盖；读-改-写全程同步，与后台任务（微博同步/Issue 更新）并发安全。
     */
    router.patch('/api/config', (req, res) => {
        try {
            ...
            for (const key of Object.keys(sections)) {
                config[key] = deepMerge(config[key], sections[key]);
            }
            if (Array.isArray(body.modules)) config.modules = body.modules;
            writeConfig(config);
            res.json({ success: true, config: readConfig() });
        } catch (err) {
            res.status(500).json({ error: 'Failed to patch config: ' + err.message });
        }
    });
