...
module.exports = function createCommentsRoutes(core) {
    ...
    router.post('/api/comments/settings', (req, res) => {
        try {
            ...
            core.writeConfig(config);
            // 即时生效：同步重写 NAS 实时站读取的 data-comments.js（与导出同口径；此前只写 config，
            // 实时站要等下次导出才看到开关变化，被站长抓到「关了还在渲染」）
            try {
                const siteJsDir = path.join(core.PATHS.SITE_DIR, 'js');
                fs.mkdirSync(siteJsDir, { recursive: true });
                fs.writeFileSync(path.join(siteJsDir, 'data-comments.js'), core.generateCommentsJs(config), 'utf-8');
            } catch (e) {
                console.error('[comments] 重写 data-comments.js 失败:', e.message);
            }
